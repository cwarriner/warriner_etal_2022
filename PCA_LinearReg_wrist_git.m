...
%% PCA and Linear Regression for wrist trials (3 mice, excluding A36)
...
%     cross_validate=1;
    ...
    %% What to plot?
    plot_lin_reg_traces=1;
    plot_lin_reg_traces_joint=1;
    plot_lin_reg_weights_heatmap=1;
    plot_in_pyr_figs=1;
    ...
    for i = 1:length(mouse_data_old)
        neuron_session_ID=[neuron_session_ID; i*ones(length(mouse_data_old(i).spikes),1)];
    ...
    %% take PCs of alt then orthogonalize all data of cocon w/ respect to those
     % PCs
        ...
        for i=1:length(conds)-1 %only do CC and Alt1, and ALSO A2 for the weights
            ...
            for b=1:num_perms_lambda
                ...
                for c=1:length(conds)
                    ...
                    Y_real{b,c}=Y; %record your real emg
    ...
    for k = 1:2 %1 for all neurons, 2 for just pyr
        ...
        for scale_it=1:length(scale_factors)
            ...
            %average over permutations
            R2_conds_scale(scale_it,:)=mean(Test_R2); %average over permutations
        end
    ...
    for k = 1:2 %1 for all neurons, 2 for just pyr
        ...
        for ii=1:length(conds)
            ...
            orthoWeights = Gram_Schmidt_Process(weights); %ONB=orthonormal basis
    ...
    for kk=1:2
        ...
        % find the mean across all 4 conds, and
        % subtract off and only project in the 2 conds (A1 and CC)
    ...
    % Use the "conditioned/compressed" data but find the PCs
    ...
    % now make ratios (ratios for EMG are further below, after EMG PCA)
    ...
    %% ratio of variance explained for emg
    ...
    %% RE-RUN ANALYSES ON MUSCLE-RELATED DATA
    % but NOT the Inact, keep that data unfiltered bc no muscle activity by
    % which to filter
    ...
    for kk=1:2
        ...
        %since using unclean Inact, only run this loop on 3 conds, 4th below
        for i=1:length(conds)-1
        ...
        %     %        elseif kk==3
        %     %            title('Var explained in Diff Conds INs cleaned');
        %            end
    ...
    filename=strcat(mouse_name,'_analysis_data_final_wrist.mat');
